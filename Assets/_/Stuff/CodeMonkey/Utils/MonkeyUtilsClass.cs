...
namespace CodeMonkey.Utils {
     ...
     * */
    public static class MonkeyUtilsClass {
        ...
        public static FunctionUpdater CreateMouseDraggingAction(int mouseButton, Action<Vector3> onMouseDragging) {
            ...
            return FunctionUpdater.Create(() => {
                ...
                if (dragging) {
                    onMouseDragging(MonkeyUtilsClass.GetMouseWorldPosition());
                }
                return false; 
            });
        }
        ...
        public static FunctionUpdater CreateMouseClickFromToAction(int mouseButton, int cancelMouseButton, Action<Vector3, Vector3> onMouseClickFromTo, Action<Vector3, Vector3> onWaitingForToPosition) {
            ...
            return FunctionUpdater.Create(() => {
                if (state == 1) {
                    if (onWaitingForToPosition != null) onWaitingForToPosition(from, MonkeyUtilsClass.GetMouseWorldPosition());
                }
                if (state == 1 && Input.GetMouseButtonDown(cancelMouseButton)) {
                    // Cancel
                    state = 0;
                }
                if (Input.GetMouseButtonDown(mouseButton) && !MonkeyUtilsClass.IsPointerOverUI()) {
                    if (state == 0) {
                        state = 1;
                        from = MonkeyUtilsClass.GetMouseWorldPosition();
                    } else {
                        state = 0;
                        onMouseClickFromTo(from, MonkeyUtilsClass.GetMouseWorldPosition());
                    }
                }
                return false; 
            });
